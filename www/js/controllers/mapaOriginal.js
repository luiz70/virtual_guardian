...
.controller("mapa",function($scope,$rootScope,$http,$ionicPopover,$timeout,$window,$ionicScrollDelegate,$interval){
  ...
  },
 {
	...
	initialize =function (){
		var r = document.createElement('script'); // use global document since Angular's $document is weak
            r.src = 'js/markerclusterer.js';
            document.body.appendChild(r);
			var r2 = document.createElement('script'); // use global document since Angular's $document is weak
            r2.src = 'js/markers.js';
	...
	$rootScope.onSPosc=function(position){
		$scope.setCarro(position.coords.latitude,position.coords.longitude);
...
//
	$scope.dibujaRadio=function (){
		...
		if(!$rootScope.switchMap.enable)$scope.circuloRadio.setMap(null);
	...
	$scope.setPosicion=function (map, lat,long) {
		...
		google.maps.event.addListener($rootScope.ubicacionMarker, 'position_changed',function(){
		if($scope.placeMarker){
			$scope.placeMarker.setMap(null);
			$scope.placeMarker=null
		}
		})
...
$scope.buscaLugar=function(){
	...
	if($scope.timesearch)$timeout.cancel($scope.timesearch);
	$scope.timesearch=$timeout($scope.searchNow,1000);
}
$scope.searchNow=function(){
  ...
  service.getPlacePredictions({ input: $scope.textoBuscado ,componentRestrictions: {country: 'mx'}}, $scope.predicciones);
...
$scope.predicciones = function(predictions, status) {
  ...
  $scope.placeDetails=function(result,status){
	  ...
	  if (status == google.maps.places.PlacesServiceStatus.OK) {
		  $scope.$apply(function () {
          	$scope.resultadoLugares.push(result);
        });
	  }
  }
...
$scope.setPlace=function (obj) {
	...
	$scope.placeMarker = new RichMarker({
      position: obj.geometry.location,
      map: $rootScope.map,
	  shadow: 'none',
      content: '<div class="placeMarker"><div class="contIconMarker"><img class="iconMarker" src="img/blank.png" style="-webkit-mask-image: url('+obj.icon+');" ></div></div>'
	});
	}
}
$scope.setCarro=function (lat,long) {
	...
	$scope.carroMarker.setMap(null);
	$scope.carroMarker=null
	console.log(2);
	}
	if(!$scope.carroMarker){
	var icono = {
...
$scope.cargaInfoEvento=function(val){
	...
	if(!$scope.InfoboxEvento)$scope.InfoboxEvento = new InfoBox({
		...
		if(val){
			...
			var s=parseFloat($scope.selectedMarker.data.Latitud)+0.04;
			console.log(s)
			$rootScope.map.setCenter(new google.maps.LatLng(parseFloat($scope.selectedMarker.data.Latitud)+0.03,$scope.selectedMarker.data.Longitud));
            }else {
            $scope.markerLoaded=false;
            $scope.selectedMarker=null;
...
$scope.setMarcador=function (evento) {
	...
	if($.grep($scope.marcadores, function(obj) { return obj.data.IdEvento == evento.IdEvento; }).length==0){
	//if(!$scope.circuloRadio)
	$scope.dibujaRadio();
...
//$scope.infos.close();
	...
	$scope.limpia=function(){
		if($scope.mc){
			$scope.mc.removeMarkers($scope.marcadores);
		/*$scope.mc.setMaxZoom(1);
$scope.mc.setGridSize(1);
$scope.mc.repaint();*/
		}
		$scope.cargaInfoEvento(false);
	...
	if($rootScope.Eventos.length>0){
	if($scope.timeMap!=null)$timeout.cancel($scope.timeMap);
	$scope.timeMap=$timeout($scope.showEv,300);
	}else $scope.limpia();
	}
	$scope.showEv=function(){
		$scope.limpia();
	...
	maxZoom:10
		...
		//if($scope.marcadores.length>=20)opt.maxZoom=17
		//else opt.maxZoom=10
		$scope.mc = new MarkerClusterer($rootScope.map,[],opt);
		
		if($scope.marcadores.length>10){
			$scope.mc.addMarkers($scope.marcadores);
		}
